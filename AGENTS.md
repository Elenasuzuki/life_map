...
- 状態管理: [src/App.jsx](src/App.jsx) で `layers`・`baseMap`・`driveMinutes`（30/35/40/45/50/55/60分、デフォルト45分、localStorage永続化）を保持
...
│   └── drive_isochrones/            車アクセス圏（事前生成、42ファイル）
│       └── {workplace}_{mode}_{minutes}min.geojson
│           workplace: toyota_hq / toyota_shimoyama / shota_family_home
│           mode: no_highway / highway
│           minutes: 30 / 35 / 40 / 45 / 50 / 55 / 60
└── scripts/
    ├── generate_walk_isochrones.py  徒歩圏 GeoJSON 生成
    ├── generate_drive_isochrones.py 車アクセス圏 GeoJSON 生成（事前生成用）
...
| `carToyotaHq` | `locations.js` + `drive_isochrones/toyota_hq_*.geojson` | トヨタ本社 車アクセス圏 |
| `carShimoyama` | `locations.js` + `drive_isochrones/toyota_shimoyama_*.geojson` | 下山TC 車アクセス圏 |
| `carShotaHome` | `locations.js` + `drive_isochrones/shota_family_home_*.geojson` | 昭太実家 車アクセス圏 |
...
- ファイル: `src/data/locations.js` + `public/data/drive_isochrones/*.geojson`
- 起点: `トヨタ本社 / トヨタテクニカルセンター下山 / 昭太実家`
- ORS `driving-car` isochrone、`smoothing: 0` で事前生成済み
- 等時間圏の分数は `driveMinutes`（デフォルト45分、LayerPanel のボタンで 30/35/40/45/50/55/60分 に変更可）
- 高速回避版（`no_highway`）と高速ありの2系統。各ファイル名: `{workplace}_{mode}_{minutes}min.geojson`
- 高速回避版の境界は同色の実線。高速あり版は半透明塗り + 点線で重ねる
- 3重なりだけ紺色の細い実線で強調（2重なり専用線はなし）
- 分数は `localStorage`（`housing-map-drive-minutes-v1`）に保存
- ファイルを追加・再生成する場合は `scripts/generate_drive_isochrones.py` を使用（`.env.local` の `VITE_ORS_API_KEY` が必要）
...
### 4. 車アクセス圏は事前生成ファイル依存
`public/data/drive_isochrones/` の42ファイルが必要。ORS APIキーは不要（ランタイム呼び出しなし）。
分数の追加や再生成が必要な場合は `scripts/generate_drive_isochrones.py` を使う。
...
- ホスティング: Vercel（GitHub連携、`main` ブランチへのプッシュで自動デプロイ）
- ビルドコマンド: `npm run build`、出力: `dist/`
- 環境変数: Vercel側での設定不要（ORS APIキーはランタイム不使用）
...
- 車アクセス圏 GeoJSON の事前生成（`scripts/generate_drive_isochrones.py`）
- ランタイムでは使用しない
